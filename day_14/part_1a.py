    ...
    for vert_dir in [1, -1]: # scans rows top to bottom then bottom to top
        for row in range(start_row, end_row, vert_dir):
                        ...
                        if test_char == search_char:
                            if col != start_col:
                                if col in row_starts:
                                    ...
                                    print_i(f"\t\t\t\t[{next_row_start_index}] row_starts was: {row_starts}")
                                    row_starts.pop(col_in_row_starts)
                                    if col_in_row_starts < col:
                                        # next_row_start_index -= 1
                                        next_row_start_index_inc -= 1
                                    print_i(f"\t\t\t\t[{next_row_start_index}] row_starts is: {row_starts}")
                            if [row, col] not in found_plants:
                                ...
                                #     if [row, col-hor_dir] not in found_walls_down:
                                #         print_i(f"\t\tside wall B at x: {col-hor_dir}, y: {row}")
                                #         found_walls.append([row, col-hor_dir])
                                #         walls += 1
                                # if col == num_cols - 2: # redundant
                                #     if [row, num_cols-1] not in found_walls_down:
            ...
            if len(next_row_starts) == 0:
                break
            next_row_start_index += next_row_start_index_inc
