...
# input_file_name = "test_input_8.txt"
...
start_row = int(start_pos / num_columns)
start_column = start_pos % num_columns
...
def test_loop(added_row, added_column, initial_dir=-1):
    global dir
    global last_dir
    global curr_row
    global curr_column
    ...
    num_turns = 0
    num_times_hit_wall = 0

    # if added_row != -1:
    #     test_row = added_row
    #     test_column = added_column
    ...
    while True:
        ...
        if not first_pass:
            ...
            # if curr_column == added_column and curr_row == added_row and num_turns > 3 and dir == initial_dir:
            # if curr_column == added_column and curr_row == added_row and num_turns > 6:
            if num_times_hit_wall > 0:
                if [curr_column, curr_row, dir] not in poses_passed:
                    poses_passed.append([curr_column, curr_row, dir])
                else:
                    num_times_hit_wall += 1
            if curr_column == added_column and curr_row == added_row and num_times_hit_wall < 1:
                # if num_loops > 0:
                #     exit(-1)
...
for coords in poses_to_test:
    ...
    if test_loop(test_row, test_column, initial_dir):
...
# answer: 1644 is wrong
# answer: 1753 is correct
